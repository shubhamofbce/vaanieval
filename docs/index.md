# Documentation Index

VaaniEval now ships with adapter-based support for both ElevenLabs and Vapi, so provider-specific behavior stays isolated while the rest of the platform stays stable.

## V2 Core

- [Development Guide](development.md)
- [Backend Architecture](backend-architecture.md)
- [Feature Playbooks For Agents](features/README.md)
- [Backend Feature Playbooks](features/backend/README.md)
- [Frontend Feature Playbooks](features/frontend/README.md)
- [V2 Plan](v2-plan/README.md)
- [V2 Roadmap](v2-plan/roadmap.md)
- [V2 Score Taxonomy](v2-plan/score-taxonomy.md)
- [V2 Audio Scalability Plan](v2-plan/audio-scalability-plan.md)

## Legacy Reference

- [Quickstart](quickstart.md)
- [Configuration](configuration.md)
- [Scenario Authoring](scenarios.md)
- [Metrics And Gates](metrics-and-gates.md)
- [Architecture](architecture.md)
- [Backend API Reference](cli-and-api.md)
- [Contributing](contributing.md)
- [Troubleshooting](troubleshooting.md)
